...
CALLOUT / CHAIN interaction system — Replace proximity events with a CALLOUT event that fires when two characters share a room (hallway or regular). Target rolls to accept or decline based on mood/traits/relationship. Acceptance locks both into an INTERACTION event (drink together, walk together, converse). At sequence end, low-probability roll to chain into another interaction (higher with good relationship, SOCIAL trait, relevant feelings). Queued events preserved; movement cancels cleanly toward target spot; chained events clear the queue. Requires: new interaction spots in room scenes, queue priority rules, movement-toward-target routing. Replaces current allow_hallway CONVERSE entirely once built.
Base event layer — Replace flat event pool with two-tier pipeline. Tier 1: high-level intent events (SOCIALIZE, VISIT_, FULFILL_NEED, EXPLORE). These roll first and resolve context — VISIT_ checks needs/traits/feelings to pick destination room; FULFILL_NEED maps hunger→kitchen, stress→bar, loneliness→social space, etc. Tier 2: specific action events fire as a result of Tier 1 resolution. Benefits: easier to tune (adjust one SOCIALIZE weight instead of 12 social event weights), more coherent character behaviour (character goes to bar BECAUSE they're stressed, not by coincidence), better foundation for future AI. Suggested for this phase after dead code cleanup and CALLOUT system. Requires: new routing in _run_pipeline, context resolver, event categorisation pass.
